...
class ConvPoolLayer():
	...
	n_in = np.prod(filter_shape[1:])
        ...
        self.w = theano.shared(
            np.asarray(
                np.absolute(np.random.normal(loc=0, scale=np.sqrt(1.0/(n_in)), size=filter_shape)),
                dtype=theano.config.floatX),
            borrow=True)
        self.b = theano.shared(
            np.asarray(
                np.absolute(np.random.normal(loc=0, scale=1.0, size=(filter_shape[0],))),
                dtype=theano.config.floatX),
            borrow=True)
