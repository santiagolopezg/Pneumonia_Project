	...
	self.mcc = []
	    ...
	    try:
	       mcc = (TP*TN - FP*FN)/(math.sqrt((TP + FP)*(TP + FN)*(TN + FP)*(TN + FN)))
 	       print("Epoch {0}: MCC {1:.2}".format(
               epoch, mcc))
     	       self.mcc.append(mcc)
	       if mcc >= best_mcc:
                    print("This is the best Matthews correlation coefficient to date.")
                    best_mcc = mcc
        ...
        #print("Best MCC of {1:.2} obtained at iteration {1}".format(
        #    best_mcc,iteration))
	self.best_mcc = best_mcc
