...
file_name = "../data/Base_de_datos_SinNeumEvid/neumonia_dataset_interson_elDeform_SinNeumEvid_8.pkl"
#file_name = "../data/Base_de_datos_alldata/neumonia_dataset_interson_elDeform_alldata_8.pkl"
########################################
# HyperParameters
startTime = datetime.now()
possible_learning_rate =[1/100.0] #5
#possible_learning_rate = [1/10000.0] #5
possible_lambda = [1.0/100.0] #7
#possible_lambda = [1.0] #7
possible_mini_batch = [28]
possible_dropout = 0.7
########################################
# Import libraries
import cPickle
import network_interson_2
from network_interson_2 import Network
from layer_types import ConvPoolLayer, ConvLayer, FullyConnectedLayer, SigmoidLayer, SoftmaxLayer
training_data, validation_data, test_data = network_interson_2.load_data_shared(filename= file_name)
from network_interson_2 import ReLU
...
for i in range(len(possible_learning_rate)):
	for j in range(len(possible_lambda)):
		for z in range(len(possible_mini_batch)):
			...
			name = 'net_trial_%(learning)g_%(lambda)g_%(mini_batch)g_%(dropout)g.pkl' %{"learning": possible_learning_rate[i],"lambda":possible_lambda[j],"mini_batch":mini_batch_size,"dropout":dropout}
			f = file(name,'wb')
