...
for jk in xrange(9):
	...
	model = Sequential()
	model.add(Dense(input_dim=(256*256),output_dim = 100,W_regularizer=l1l2(l1 = 0.01,l2=0.001)))
	model.add(Dense(output_dim = 10,W_regularizer=l1l2(l1 = 0.01,l2=0.001)))
	model.add(Dense(output_dim=1, W_regularizer=keras.regularizers.l2(1.)))
	...
	#Safe weights
	name = 'neumonia_dataset_interson_keras_alldata_{0}_weights_fc_{0}.h5'.format(number_db,(number_db))
	print(name)
	...
	'''
	import cPickle
	f = open('fullyconnected_loss_{0}.pkl'.format(number_db),'wb')
	cPickle.dump(history.losses,f,protocol=cPickle.HIGHEST_PROTOCOL)
	f.close()
	
	h = open('fullyconnected_metrics_{0}.pkl'.format(number_db),'wb')
